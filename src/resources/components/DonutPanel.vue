<template>
    <pane v-bind:title="title">
        <div class="commerce-insights-donut-pane" :style="{ 'min-width: 240px;': isVertical, 'min-width: 320px;': ! isVertical }">
            <div v-if="isVertical">
                <div ref="chartWrapper" class="commerce-insights-donut-chart-wrapper w-64 max-h-64 max-w-full">
                    <chart type="donut" 
                ...
                <table class="data fullwidth mini-table">
                    <tbody>
                        <tr v-for="(item, index) in data" v-bind:key="item.title">
                            <td><div class="color-key" v-bind:style="{ 'background': backgroundColors[index] }"></div></td>
                            <td>{{ item.title }}</td>
