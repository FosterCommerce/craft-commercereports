                ...
                <div class="flex w-1/2">
                    <!--<div class="">
                        ...
                        <div class="menu">
                            <ul class="padded">
                                <li>
                                    <a :class='{ "sel": Object.keys(selectedTypeOption).length === 0 }'
                                        v-on:click="selectAllTypes()">{{ allTypesLabel }}</a>
                                </li>
                                <li v-for="option in typeOptions" v-bind:key="option.value">
                                    <a :class='{ "sel": selectedTypeOption.value == option.value }'
                                        v-on:click="selectTypeOption(option)">{{ option.label }}</a>
                                </li>
                            </ul>
                        </div>
                    </div>-->
                    <div class="flex-grow texticon search icon clearable">
                        <input class="text fullwidth" v-model="keyword" @input="resetPage" type="text" autocomplete="off" :placeholder="getPlaceholder()">
                    ...
                    <table class="data fullwidth">
                        ...
                        <tbody v-if="elementType === 'Orders'">
                            ...
                            <tr v-for="element in filteredElements" v-bind:key="element.id">
                                ...
                                <!--<td>
                                    <span class="status" :class="element.paidColor"></span>{{ element.paymentStatus }}
                                </td>-->
                            </tr>
                        </tbody>
    ...
    methods: {
        getSortBy() {
            switch (this.elementType) {
                case 'Orders': this.sort_by = 'dateStamp';
                    break;
                case 'Sales': this.sort_by = 'totalSold';
                    break;
                case 'Products': this.sort_by = 'title';
                    break;
                case 'Customers': this.sort_by = 'lastPurchase';
                    break;
            }
        },
        getPlaceholder() {
            switch (this.elementType) {
                ...
                case 'Sales': return 'Search sales';
                case 'Products': return 'Search product name or SKU';
                    ...
                    switch(this.sort_by) {
                        ...
                        default:
                            if(this.sort_direction === 'desc') {
                                if(a_sort_by < b_sort_by) { return 1; }
                                if(a_sort_by > b_sort_by) { return -1; }
                            } else {
                                if(a_sort_by < b_sort_by) { return -1; }
                                if(a_sort_by > b_sort_by) { return 1; }
